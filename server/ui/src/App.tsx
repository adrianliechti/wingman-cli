...
export default function App() {
	...
	const paletteActions: PaletteAction[] = (() => {
		...
		for (const backend of client.scope.backends) {
			const name = formatAgentName(backend.id, backend.name);
			actions.push({
				id: `new-session:${backend.id}`,
				label:
					client.scope.backends.length > 1 ? `New Chat (${name})` : "New Chat",
				icon: <MessageSquarePlus size={12} className="text-fg-dim shrink-0" />,
				run: () => void handleNewSession(backend.id),
			});
		}
		if (tabs.some((tab) => tab.type === "chat"))
			actions.push({
				id: "show-sessions",
				label: "Show sessions",
