...
// sandboxDenialKeywords catches common phrasing OS sandboxes and the shells
// running under them use for a denied filesystem operation.
...
// isLikelySandboxDenied guesses whether a failed command was blocked by the
// workspace sandbox rather than failing on its own merits. There is no fully
// reliable signal — a command can print "permission denied" for reasons that
// have nothing to do with the sandbox — so this stays conservative: it only
// looks at well-known non-sandbox shell exit codes and keyword matches in the
// captured output, mirroring the heuristic Codex CLI uses for the same
// problem (see codex-rs/sandboxing/src/denial.rs).
func isLikelySandboxDenied(exitCode int, output string) bool {
	if exitCode <= 0 {
		return false
	}

	// 2: misuse of shell builtins, 126: not executable, 127: command not
	// found — all unrelated to sandbox enforcement.
	switch exitCode {
	case 2, 126, 127:
		return false
	}
