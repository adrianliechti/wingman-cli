---
name: pull-request
description: Prepare, push, create, or update a pull request from the current work with a focused commit history, verified branch diff, accurate rationale, test evidence, and no unrelated or sensitive files.
when-to-use: When the user asks to open, create, prepare, publish, or refresh a pull request; invocation authorizes the necessary branch push and PR mutation but never force-pushing or merging.
arguments: [title]
---
# Pull Request
...
`${title}` is an optional title or intent hint.
...
If on the default branch with uncommitted intended work, create a concise descriptive branch. Stage explicit paths only and inspect `git diff --cached`. Commit using the repository's message style, incorporating `${title}` when useful. Preserve existing commits and staged work; never amend or rewrite history.
