...
export function Sidebar({ currentSessionId, onSessionSelect, onNewSession }: Props) {
  ...
  return (
    <div className="w-56 flex flex-col bg-bg shrink-0 border-r border-border-subtle">
      {/* Header */}
      <div className="h-10 px-3 flex items-center justify-between shrink-0">
        <span className="text-[11px] font-medium text-fg-dim uppercase tracking-wider">Sessions</span>
        <button
      ...
      <div className="flex-1 overflow-y-auto pb-2">
        {groups.length === 0 && (
          <div className="px-3 py-8 text-[11px] text-fg-dim text-center">No sessions yet</div>
        )}
        {groups.map(group => (
          <div key={group.label}>
            <div className="px-3 pt-4 pb-1.5">
              <span className="text-[10px] font-medium uppercase tracking-wider text-fg-dim">{group.label}</span>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[12px] leading-snug">{displayTitle}</div>
                    <div className="text-[10px] text-fg-dim truncate mt-0.5">{relativeTime(s.updated_at)}</div>
                  </div>
                  <button
